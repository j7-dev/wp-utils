...
3. **DTO 類別範例** - Txn.php, Wallet.php 展示了資料傳輸物件的實作方式
...
### 步驟 1: 設計 CRUD 介面
創建 `src/interfaces/CrudInterface.php`
- 定義標準的 CRUD 方法簽名
- 定義查詢方法介面

### 步驟 2: 實作基礎 CRUD 抽象類
創建 `src/abstracts/BaseCRUD.php`
- 實作通用的 CRUD 操作
- 提供靈活的查詢建構器
- 處理資料庫連接和錯誤處理
- 整合現有的 Table helper
...
### 步驟 3: 創建對應的 CRUD 類
- 創建 `src/services/Log/CRUDs/TxnCRUD.php` 繼承 BaseCRUD
- 創建 `src/services/Log/CRUDs/WalletCRUD.php` 繼承 BaseCRUD
- 整合現有的 DTO 類別（`src/services/Log/DTOs/` 下的 Txn.php, Wallet.php）

### 步驟 4: 提供工廠模式支援
創建 CRUD Factory 簡化實例化
...
// Read
$txn = $txnCrud->find(1);
$transactions = $txnCrud->findBy(['wallet_id' => 1]);

// Update
$success = $txnCrud->update(1, ['title' => 'Updated Title']);

// Delete
$success = $txnCrud->delete(1);
...
建立一個強大、靈活、易用的 BaseCRUD 抽象類，讓開發者可以：
