...
final class WPUPointUtils {
	...
	public const DEFAULT_POINT_SLUG = 'wpu_default_point';
	...
	public function register_post_type(): void {
		...
		$labels = [
			...
			'featured_image'        => _x( 'Point Cover Image',
			...
			'filter_items_list'     => _x( 'Filter books list',
				...
				'wp-utils' ),
			'items_list_navigation' => _x( 'Points list navigation',
	...
	public function get_default_point(): ?WPUPoint {
		return self::get_point_by_slug( self::DEFAULT_POINT_SLUG );
	}
	...
	public function create_default_point(): void {
		...
		$page = \get_page_by_path( self::DEFAULT_POINT_SLUG, OBJECT, $post_type );
		if ( $page ) {
			return;
		} else {
			...
			$post_id = \wp_insert_post(
				[
					...
					'post_type'   => $post_type,
					'post_status' => 'publish',
					'post_name'   => self::DEFAULT_POINT_SLUG,
				]
			);
			// TODO 設定預設圖片
		}
