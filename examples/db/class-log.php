...
namespace J7\PowerPartnerServer\Utils;

use J7\PowerPartnerServer\Plugin;
	...
	public static function insert_user_log( $user_id = 0, $args = array(), $points = 0, $points_slug = '' ) {
		...
		try {
			...
			if ( ! $result ) {
				...
				\J7\PowerPartnerServer\Debug::debug_log( '' . ob_get_clean() );
			}
		} catch ( \Throwable $th ) {
			ob_start();
			print_r( $th );
			\J7\PowerPartnerServer\Debug::debug_log( 'insert log Error ' . ob_get_clean() );
		}
	}
}
